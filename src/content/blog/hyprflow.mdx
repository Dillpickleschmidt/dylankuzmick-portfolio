...
video: "https://youtu.be/-yLSomG0_vU"
---
...
It gets worse. Browsers treat all of `localhost` as a single origin — `localhost:3000` and `localhost:5432` share the same cookies, localStorage, IndexedDB, and Service Workers. Ports don't matter. And even if you solved the single-process problem, namespace isolation alone wouldn't be enough — a shared browser profile stores all `localhost` cookies and localStorage in one place, so Group 1's auth tokens and Group 2's auth tokens would still collide.

Hyprflow solves both problems at once using [firejail](https://github.com/netblue30/firejail), a Linux sandboxing tool that can launch a process inside a specific network namespace. Each group gets its own browser process in its own namespace, with a separate profile directory so cookies and localStorage are isolated too:
...
A notification proxy prepends the group and workspace slot to every notification. When Claude Code finishes a task on the first workspace of Group 2, the notification says `[2-1] Task complete` — you know immediately which project it's from without hunting through tabs.
